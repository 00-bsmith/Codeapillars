...
@Repository
public class MediumScoreJdbcTemplateRepository implements ScoreRepository{

    private final JdbcTemplate jdbcTemplate;

    public MediumScoreJdbcTemplateRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public List<ScoreEntry> findAll() {
        final String sql="select score_id, initials, score, date from medium_high_score;";
    ...
    public ScoreEntry add(ScoreEntry entry) {
        final String sql = "insert into medium_high_score (initials, score, date) values (?, ?, ?);";
    ...
    public boolean update(ScoreEntry entry) {
        final String sql = "update medium_high_score set " +
                "initials = ?, " +
                "score = ?, " +
                "date=? " +
                "where score_id = ?;";
    ...
    public List<ScoreEntry> findRanking() {
        final String sql = "select score_id, initials, score, date from medium_high_score order by score desc, `date` asc limit 50;";
        return jdbcTemplate.query(sql,new ScoreMapper());
    }
}
