...
import capstone.trivia_game.data.LongScoreJdbcTemplateRepository;
...
public class LongScoreService {
    ...
    private final LongScoreJdbcTemplateRepository repository;

    public LongScoreService(LongScoreJdbcTemplateRepository repository) {
    ...
    private Result<ScoreEntry> validate(ScoreEntry scoreEntry) {
        ...
        if (scoreEntry.getScore() > 3000) {
            result.addMessage("Score cannot be greater than 3000", ResultType.INVALID);
        }
