...
import capstone.trivia_game.data.MediumScoreJdbcTemplateRepository;
...
@Service
public class MediumScoreService {

    private final MediumScoreJdbcTemplateRepository repository;

    public MediumScoreService(MediumScoreJdbcTemplateRepository repository) {
    ...
    private Result<ScoreEntry> validate(ScoreEntry scoreEntry) {
        ...
        if (scoreEntry.getScore() > 1500) {
            result.addMessage("Score cannot be greater than 1500", ResultType.INVALID);
        }
