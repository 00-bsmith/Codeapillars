...
public class LongScoreJdbcTemplateRepository implements ScoreRepository{
    ...
    public LongScoreJdbcTemplateRepository(JdbcTemplate jdbcTemplate) {
    ...
    public List<ScoreEntry> findAll() {
        ...
        final String sql="select score_id, initials, score, date from long_high_score;";
    ...
    public ScoreEntry add(ScoreEntry entry) {
        final String sql = "insert into long_high_score (initials, score, date) values (?, ?, ?);";
    ...
    public boolean update(ScoreEntry entry) {
        final String sql = "update long_high_score set " +
                "initials = ?, " +
                "score = ?, " +
                "date=? " +
                "where score_id = ?;";
    ...
    public List<ScoreEntry> findRanking() {
        final String sql = "select score_id, initials, score, date from long_high_score order by score, `date` desc limit 50;";
        return jdbcTemplate.query(sql,new ScoreMapper());
    }
}
